...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const [existingUser] = (await connection.query('SELECT id FROM campaigns WHERE id = ?', [
      id,
    ...
    if (type === 'disable') {
      // Disable community
      await connection.query('UPDATE campaigns SET status = 0 WHERE id = ?', [id])
      message = 'Campaign disabled successfully'
    } else if (type === 'enable') {
      // Enable Sponsors
      await connection.query('UPDATE campaigns SET status = 1 WHERE id = ?', [id])
