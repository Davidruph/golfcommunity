...
export async function GET(request: Request) {
  ...
  try {
    ...
    const countQuery = `
      SELECT COUNT(DISTINCT events.id) as total 
      FROM events 
      ${whereClause}
    `
    ...
    const dataQuery = `
      SELECT 
        ...
        ${currentUserId ? `MAX(CASE WHEN event_attendees.user_id = ${currentUserId} THEN 1 ELSE 0 END)` : '0'} AS user_event_status
      FROM events 
...
export async function PATCH(req: NextRequest) {
  ...
  try {
    ...
    const feeLink = formData.feeLink as string | null
    const totalAllowedSpots = formData.totalAllowedSpots as number | null
    const eventId = formData.id as number
    ...
    if (event_name && eventId) {
      const [existingEvent] = (await connection.query(
        'SELECT id FROM events WHERE event_name = ? AND id != ?',
        [event_name, eventId]
      )) as [Array<RowDataPacket>, unknown]
    ...
    await connection.query('UPDATE events SET ? WHERE id = ?', [eventData, eventId])
    ...
    return NextResponse.json(
      {
        error: 'Database error',
        message: (err as Error)?.message || 'An error occurred during event update',
      },
      { status: 500 }
    )
  }
