...
export async function PATCH(req: NextRequest) {
  ...
  try {
    ...
    const exisingImagePath = existingCommunityData.banner_image
    const newimagePath = body.bannerImage

    let imagePath

    if (exisingImagePath && exisingImagePath !== newimagePath) {
      imagePath = body.bannerImage
    } else {
      imagePath = exisingImagePath
    }
